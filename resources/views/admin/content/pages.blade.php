...
@section('title', 'Pages Management - WorkNepal Admin')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['total'] ?? $pages->total() }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">{{ $stats['published'] ?? $pages->where('is_published', true)->count() }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-yellow-600">{{ $stats['draft'] ?? $pages->where('is_published', false)->count() }}</p>
                    </div>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                                        <a href="{{ route('admin.content.pages.edit', $page) }}" class="hover:text-red-600">
                                            {{ $page->title }}
                                        </a>
                                    </div>
                                    <div class="text-xs text-gray-500">ID: {{ $page->id }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($page->is_published)
                                        ...
                                        @if($page->published_at)
                                            <div class="text-xs text-gray-500 mt-1">{{ $page->published_at->format('M d, Y') }}</div>
                                        @endif
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <form method="POST" action="{{ route('admin.content.pages.toggle-status', $page) }}" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                    class="{{ $page->is_published ? 'text-yellow-600 hover:text-yellow-900' : 'text-green-600 hover:text-green-900' }} dark:{{ $page->is_published ? 'text-yellow-400' : 'text-green-400' }}"
                                        ...
                                        <form method="POST" action="{{ route('admin.content.pages.delete', $page) }}" 
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-gray-600 dark:text-gray-400">Create your first page to get started</p>
                                    <a href="{{ route('admin.content.pages.create') }}" 
                                       class="mt-4 inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                        </svg>
                                        Create New Page
