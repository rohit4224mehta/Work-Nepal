...
@section('meta_description', 'Find verified jobs in Nepal. 5000+ active jobs, 1000+ companies, fresher friendly, foreign employment with safety info. Join Nepal\'s fastest growing job platform.')
...
@section('meta_og_description', '5000+ verified jobs • 1000+ companies • Fresher friendly • Safe foreign employment')
...
<section class="relative bg-gradient-to-br from-red-600 via-red-500 to-orange-500 text-white overflow-hidden">
    ...
    <div class="container mx-auto px-4 py-24 md:py-32 relative z-10">
        <div class="max-w-4xl mx-auto text-center">
            {{-- Trust Badge --}}
            <div class="inline-flex items-center px-4 py-2 bg-white/20 backdrop-blur-lg rounded-full mb-8">
                <svg class="w-4 h-4 text-green-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                </svg>
                <span class="text-sm font-medium">Trusted by {{ number_format($stats['total_users']) }}+ Job Seekers & {{ number_format($stats['companies_count']) }}+ Companies</span>
            </div>
            ...
            <p class="text-xl md:text-2xl mb-10 max-w-3xl mx-auto font-light opacity-90">
                {{ number_format($stats['active_jobs']) }}+ verified jobs • {{ number_format($stats['fresher_jobs']) }}+ fresher friendly • {{ number_format($stats['foreign_jobs']) }}+ foreign jobs with safety info
            </p>

            {{-- Search Bar --}}
            <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl p-4 md:p-6">
                ...
                <div class="flex flex-wrap items-center gap-3 mt-4 text-sm">
                    ...
                    @foreach($popularSearches as $search)
                        <a href="{{ route('jobs.index', ['q' => $search]) }}" 
        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div class="text-center group hover:transform hover:-translate-y-1 transition">
                    <div class="text-4xl md:text-5xl font-bold text-red-600 mb-2 counter" data-target="{{ $stats['active_jobs'] }}">0</div>
                    <p class="text-gray-600 font-medium">Active Jobs</p>
                </div>
                <div class="text-center group hover:transform hover:-translate-y-1 transition">
                    <div class="text-4xl md:text-5xl font-bold text-red-600 mb-2 counter" data-target="{{ $stats['companies_count'] }}">0</div>
                    <p class="text-gray-600 font-medium">Companies</p>
                </div>
                <div class="text-center group hover:transform hover:-translate-y-1 transition">
                    <div class="text-4xl md:text-5xl font-bold text-red-600 mb-2 counter" data-target="{{ $stats['freshers_hired'] }}">0</div>
                    <p class="text-gray-600 font-medium">Freshers Hired</p>
                </div>
                <div class="text-center group hover:transform hover:-translate-y-1 transition">
                    <div class="text-4xl md:text-5xl font-bold text-red-600 mb-2 counter" data-target="{{ $stats['total_users'] }}">0</div>
    ...
    <section class="py-20 bg-gray-50">
        <div class="container mx-auto px-4">
            <div class="text-center max-w-3xl mx-auto mb-12">
                ...
                <p class="text-xl text-gray-600">Find your perfect role from thousands of opportunities</p>
            </div>

            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                @foreach($categories as $category)
                    <a href="{{ route('jobs.index', ['category' => $category['slug']]) }}" 
                       class="group bg-white rounded-xl p-6 text-center hover:shadow-lg transition border border-gray-200">
            ...
            <div class="text-center mt-8">
                <a href="{{ route('jobs.index') }}" class="text-red-600 hover:text-red-700 font-semibold inline-flex items-center">
                    View All Categories
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                @foreach($locations as $location)
                    <a href="{{ route('jobs.index', ['location' => $location['name']]) }}" 
                       class="group bg-gray-50 rounded-xl p-6 text-center hover:shadow-lg transition border border-gray-200">
            ...
            <div class="container mx-auto px-4">
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($featuredJobs as $job)
                        <div class="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all border border-gray-200 overflow-hidden group">
                            <div class="p-6">
                                {{-- Company Header --}}
                                <div class="flex items-start gap-4 mb-4">
                                    <div class="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center overflow-hidden">
                                        @if($job->company && $job->company->logo_path)
                                            <img src="{{ $job->company->logo_url }}" alt="{{ $job->company->name }}" class="w-full h-full object-cover" loading="lazy">
                                        @else
                                    ...
                                    @if($job->is_featured)
                                        <span class="px-2 py-1 bg-yellow-100 text-yellow-700 text-xs rounded-full">Featured</span>
                                    @endif
                                </div>
    ...
    {{-- NEW: Featured Employers Section --}}
    @if($featuredEmployers->isNotEmpty())
        <section class="py-20 bg-gray-50">
            <div class="container mx-auto px-4">
                <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-12">
                    <div>
                           ...
                           class="group bg-white rounded-xl p-6 text-center hover:shadow-lg transition border border-gray-200">
                            <div class="w-16 h-16 mx-auto mb-4 rounded-xl bg-gray-100 flex items-center justify-center overflow-hidden group-hover:scale-110 transition">
                                @if($employer->logo_path)
                                    <img src="{{ $employer->logo_url }}" alt="{{ $employer->name }}" class="w-full h-full object-cover" loading="lazy">
                                @else
                            ...
                            <p class="text-xs text-gray-500">{{ $employer->job_postings_count }} open jobs</p>
                ...
                <div class="grid md:grid-cols-3 gap-8">
                    @foreach($testimonials as $testimonial)
                        <div class="bg-gray-50 rounded-2xl p-8 relative">
                            ...
                            {{-- Content --}}
                            <p class="text-gray-700 mb-6 relative z-10">"{{ $testimonial->content }}"</p>
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-sm text-gray-500">{{ $testimonial->user->headline ?? $testimonial->user->job_title ?? 'Job Seeker' }}</p>
                                </div>
                            </div>
    ...
    <section class="py-20 bg-gray-900 text-white">
        <div class="container mx-auto px-4 text-center">
            ...
            <p class="text-xl text-gray-300 mb-10 max-w-3xl mx-auto">Join {{ number_format($stats['total_users']) }}+ job seekers who found their dream jobs through WorkNepal</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if ('loading' in HTMLImageElement.prototype) {
        ...
        // You could implement a lazy loading library here
    }
});
</script>
@endpush